...
class MagnitudeAXI4Tester(
  dut        : MagnitudeAXI4[FixedPoint] with StandaloneAXI4Block,
  params     : LogMagnitudeParams[FixedPoint],
  sampleSize : Int,
  verbose    : Boolean = true,
  random     : Boolean = true,
  address    : AddressSet
) extends PeekPokeTester(dut.module) with OptionalAXI4MasterModel with TestUtils with SignalUtils {

  val mod: LazyModuleImp = dut.module

  // Bind nodes
  def memAXI: Option[AXI4Bundle] = if (dut.ioMem.isDefined) Some(dut.ioMem.get) else None
  ...
  for (select <- selectSeq) {
    ...
    // Write to memory
    if (memAXI.isDefined) memWriteWord(address.base, select)
    step(5)
    // Assert out.ready
