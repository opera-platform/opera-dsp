...
class RegSorterCellIO[T <: Data: Real](params: LISParams[T], index: Int) extends Bundle {
  ...
  val i_previous_next_data      = if (index == 1 || index == (params.maxWindowSize + 1)) None else Some(Input(params.dataType))
  val o_next_data               = if (index == (params.maxWindowSize + 1)) None else Some(Output(params.dataType))
