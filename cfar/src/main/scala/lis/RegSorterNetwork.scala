...
class RegSorterNetwork [T <: Data: Real] (val params: LISParams[T]) extends Module {
  ...
  val w_compacted_data_by_cell     = Wire(Vec(params.maxWindowSize, params.dataType))
  val w_kept_before_insert_by_cell = Wire(Vec(params.maxWindowSize, Bool()))

  // The register sorter network is combinational. Each cell removes one matching
  // oldest value from the sorted vector, then forwards insertion-position state
  // to the next cell so the new sample is inserted exactly once.
  cellIndices.foreach {
    case (index) => {
      ...
      if (index == 0) {
        ...
        w_compacted_data_by_cell(index)              := u_register_cell.io.o_compacted_data.get
        w_kept_before_insert_by_cell(index)          := u_register_cell.io.o_kept_before_insert
      }
      else if (index == params.maxWindowSize) {
        // Final sentinel catches an insertion that belongs after all active
        // values; it does not participate in removal/compaction.
        ...
        u_register_cell.io.i_current_sorted_data     := io.i_sorted_data(index)
        u_register_cell.io.i_next_sorted_data.get    := io.i_sorted_data(index + 1)
        u_register_cell.io.i_previous_compacted_data.get := w_compacted_data_by_cell(index - 1)
        w_compacted_data_by_cell(index)              := u_register_cell.io.o_compacted_data.get
        w_kept_before_insert_by_cell(index)          := u_register_cell.io.o_kept_before_insert
        io.o_next_sorted_data(index)                 := u_register_cell.io.o_data
      }
