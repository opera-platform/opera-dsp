...
object CFARType {
  ...
  def isValid(cfarType: Int): Boolean =
    cfarType == CellAveraging || cfarType == OrderedStatistic
}
...
case class CFARParams[T <: Data: Real](
  ...
  if (isOrderedStatistic) {
    require(maxReferenceCells > 1, "ordered-statistic CFAR requires maxReferenceCells > 1")
