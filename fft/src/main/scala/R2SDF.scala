...
class R2SDF[T <: Data: Real: Ring: BinaryRepresentation](
  val params: RadixParams[T],
) extends Module {
  require(params.delay == params.stageSize / 2, s"R2SDF expects delay = stageSize / 2, got delay=${params.delay}, stageSize=${params.stageSize}")
