...
class BitReverseTester(
  ...
  val input = inData1 ++ inData2//if (dut.params.bitReverseDir) bitrevorder_data(inData) else inData
  val output = bitrevorder_data(inData1) ++ bitrevorder_data(inData2)
  ...
  // TODO: peek real / imag
  while (read_counter < 2*sampleSize && write_counter < 6*sampleSize) {
    // Randomize ready
    ...
    if (peek(dut.io.in.valid) == 1 && peek(dut.io.in.ready) == 1) {
      poke(dut.io.in.bits.real.asSInt, input(write_counter % (2*sampleSize)))
      poke(dut.io.in.bits.imag.asSInt, input(write_counter % (2*sampleSize)))
    ...
    if (peek(dut.io.out.valid) == 1 && peek(dut.io.out.ready) == 1) {
      ...
      // Expected values
      val expected = output(read_counter % (2*sampleSize))
      val expectedLast = if (read_counter == sampleSize - 1) BigInt(1) else BigInt(0)

      // Print if enabled
      if (verbose) {
        val in = input(read_counter % (2*sampleSize))
