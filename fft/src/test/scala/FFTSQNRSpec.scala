...
/**
 * Characterizes FFT DUT SQNR against a floating-point FFT reference.
 */
class FFTSQNRSpec extends AnyFlatSpec with ChiselScalatestTester with TestConfigSupport {
  ...
  private case class SQNRCase(
    ...
    def params: FFTParams =
      FFTModelTestUtils.fftParams(radix, size, decimation, dataWidth = dataWidth, binPoint = binPoint, twiddleWidth = twiddleWidth, growEnable = growEnable)

    def plotName: String =
      Seq("fft-sqnr-fmcw-target-chirp", radix.label, decimation, size, s"$dataWidth.$binPoint", growName).mkString("-")
  ...
  private val precisionCases = for {
    ...
    check        = "measure FFT SQNR for fixed-point precision",
    radix        = radix,
  ...
  private val growthCases = for {
    ...
    check        = "measure FFT SQNR with stage growth pattern",
    radix        = radix,
    decimation   = decimation,
    size         = size,
    growName     = name,
